...
void PlayerbotHolder::AddPlayerBot(ObjectGuid playerGuid, uint32 masterAccountId)
{
    ...
    if (masterAccountId && masterPlayer)
    {
        ...
        int32 maxAddedBots = sPlayerbotAIConfig->maxAddedBots;
        if (maxAddedBots >= 0 && count >= static_cast<uint32>(maxAddedBots))
        {
            allowed = false;
            out << "Failure: You have added too many bots (more than " << maxAddedBots << ")";
        }
    }
    if (!allowed)
    {
