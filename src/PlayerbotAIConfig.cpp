...
bool PlayerbotAIConfig::Initialize()
{
    ...
    // Cargar cuentas configuradas para forzar conexión de bots
    LoadList<std::vector<uint32>>(sConfigMgr->GetOption<std::string>("AiPlayerbot.ConnectedAccounts", ""), connectedAccounts);
...
std::vector<std::vector<uint32>> PlayerbotAIConfig::ParseTempTalentsOrder(uint32 cls, std::string tab_link)
{
    ...
    for (size_t tab = 0; tab < 3u; ++tab)
    {
        if (tab >= tab_links.size())
        {
            break;
        }
        uint32 tabIndex = static_cast<uint32>(tab);
        auto& tabSpells = spells[tabIndex];
        std::sort(tabSpells.begin(), tabSpells.end(),
        ...
        for (size_t i = 0; i < tab_links[tab].size(); ++i)
        {
            ...
            if (lvl == 0)
                continue;
            orders[tab].push_back({tabIndex, tabSpells[i]->Row, tabSpells[i]->Col, static_cast<uint32>(lvl)});
        }
    }
...
std::vector<std::vector<uint32>> PlayerbotAIConfig::ParseTempPetTalentsOrder(uint32 spec, std::string tab_link)
{
    ...
    for (size_t i = 0; i < tab_link.size(); ++i)
    {
        ...
        if (lvl == 0)
            continue;
        orders.push_back({spells[i]->Row, spells[i]->Col, static_cast<uint32>(lvl)});
    }
    // sort by talent tab size
    std::sort(orders.begin(), orders.end(), [&](auto& lhs, auto& rhs) { return lhs.size() > rhs.size(); });
