...
void PlayerbotAI::UpdateAI(uint32 elapsed, bool minimal)
{
    ...
    // Alt bots should log out shortly after leaving the master's group.
    if (IsAlt())
    {
        bool inMasterGroup = false;
        if (master)
        {
            if (Group* group = bot->GetGroup())
                inMasterGroup = group->IsMember(master->GetGUID());
        }

        if (!inMasterGroup)
        {
            if (altLeftGroupTime == 0)
                altLeftGroupTime = time(nullptr);

            double secondsOut = difftime(time(nullptr), altLeftGroupTime);
            if (secondsOut >= 10.0)
            {
                if (GetMaster())
                {
                    ...
                    msg << "Bot " << bot->GetName()
                        << " desconectado por estar fuera de tu grupo mas de 10 segundos.";
                    TellMaster(msg.str());
                }

                PlayerbotMgr* masterBotMgr = master ? GET_PLAYERBOT_MGR(master) : nullptr;
                if (masterBotMgr)
