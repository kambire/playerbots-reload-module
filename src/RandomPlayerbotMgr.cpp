...
RandomPlayerbotMgr::RandomPlayerbotMgr() : PlayerbotHolder(), processTicks(0)
{
    ...
    // Forzar conexión de bots de cuentas configuradas
    if (!sPlayerbotAIConfig->connectedAccounts.empty())
    {
        LOG_INFO("playerbots", "Forzando conexión de bots de cuentas configuradas: {}", sPlayerbotAIConfig->connectedAccounts.size());
        for (uint32 accountId : sPlayerbotAIConfig->connectedAccounts)
        {
            // Buscar todos los personajes de la cuenta
            QueryResult result = CharacterDatabase.Query("SELECT guid, name FROM characters WHERE account = {}", accountId);
            if (result)
            {
                do
                {
                    ...
                    // Si el bot no está ya en línea, agregarlo a la lista de bots a conectar
                    if (!GetPlayerBot(guid))
                    {
                        currentBots.push_back(guid);
                        LOG_INFO("playerbots", "Agregando bot forzado: {} (GUID: {}) de cuenta {}", name.c_str(), guid, accountId);
                    }
...
void RandomPlayerbotMgr::UpdateAIInternal(uint32 elapsed, bool /*minimal*/)
{
    ...
    {
        if (time(nullptr) > (LfgCheckTimer + 120))
...
{
    if (!LfgCheckTimer || time(nullptr) > (LfgCheckTimer + 120))
