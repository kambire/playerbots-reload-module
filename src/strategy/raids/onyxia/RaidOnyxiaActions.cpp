...
bool RaidOnyxiaMoveToSideAction::Execute([[maybe_unused]] Event event)
{
    ...
    if (distance <= 30.0f && (diff < M_PI / 4 || diff > 3 * M_PI / 4))
    {
        ...
        // bot->Yell("Too close to front or tail â€” moving to side of Onyxia!", LANG_UNIVERSAL);
...
bool RaidOnyxiaSpreadOutAction::Execute([[maybe_unused]] Event event)
{
    ...
    // bot->Yell("Spreading out â€” I'm the Fireball target!", LANG_UNIVERSAL);
...
bool OnyxiaAvoidEggsAction::Execute([[maybe_unused]] Event event)
{
    ...
    // bot->Yell("Too close to eggs â€” backing off!", LANG_UNIVERSAL);
