...
public:
    ...
    virtual bool Before(Action* action, [[maybe_unused]] Event event) = 0;
    virtual bool AllowExecution(Action* action, [[maybe_unused]] Event event) = 0;
    virtual void After(Action* action, bool executed, [[maybe_unused]] Event event) = 0;
    virtual bool OverrideResult(Action* action, bool executed, [[maybe_unused]] Event event) = 0;
};
...
public:
    ...
    bool Before(Action* action, [[maybe_unused]] Event event) override;
    bool AllowExecution(Action* action, [[maybe_unused]] Event event) override;
    void After(Action* action, bool executed, [[maybe_unused]] Event event) override;
    bool OverrideResult(Action* action, bool executed, [[maybe_unused]] Event event) override;
...
public:
    ...
    ActionResult ExecuteAction(std::string const name, [[maybe_unused]] Event event = Event(), std::string const qualifier = "");
...
private:
    bool MultiplyAndPush(NextAction** actions, float forceRelevance, bool skipPrerequisites, [[maybe_unused]] Event event,
                         const char* pushType);
    void Reset();
    void ProcessTriggers(bool minimal);
    void PushDefaultActions();
    void PushAgain(ActionNode* actionNode, float relevance, [[maybe_unused]] Event event);
    ActionNode* CreateActionNode(std::string const name);
    Action* InitializeAction(ActionNode* actionNode);
    bool ListenAndExecute(Action* action, [[maybe_unused]] Event event);
