...
bool CastAuraSpellAction::isUseful()
{
    ...
    if (!aura)
        return true;
    if (beforeDuration && aura->GetDuration() >= 0 &&
        static_cast<uint32>(aura->GetDuration()) < beforeDuration)
        return true;
    return false;
}
...
bool BuffOnPartyAction::Execute([[maybe_unused]] Event event)
{
...
bool CastCrowdControlSpellAction::Execute([[maybe_unused]] Event event) { return botAI->CastSpell(getName(), GetTarget()); }
...
bool CastVehicleSpellAction::Execute([[maybe_unused]] Event event)
{
    uint32 spellId = AI_VALUE2(uint32, "vehicle spell id", spell);
    return botAI->CastVehicleSpell(spellId, GetTarget());
}

bool UseTrinketAction::Execute([[maybe_unused]] Event event)
{
