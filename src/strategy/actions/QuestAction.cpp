...
bool QuestAction::Execute([[maybe_unused]] Event event)
{
...
bool QuestUpdateCompleteAction::Execute([[maybe_unused]] Event event)
{
...
*/
bool QuestUpdateAddKillAction::Execute([[maybe_unused]] Event event)
{
    WorldPacket p(event.getPacket());
    p.rpos(0);
...
bool QuestUpdateAddItemAction::Execute([[maybe_unused]] Event event)
{
    WorldPacket p(event.getPacket());
    p.rpos(0);
...
bool QuestItemPushResultAction::Execute([[maybe_unused]] Event event)
{
    ...
    for (uint16 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        for (int i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
        {
            ...
            if (itemId == itemEntry && previousCount < required)
            {
                if (botAI->GetMaster())
                {
                    std::string itemLink = ChatHelper::FormatItem(proto);
                    std::ostringstream out;
                    int32 available = std::min(static_cast<int32>(itemCount), required);
...
bool QuestUpdateFailedTimerAction::Execute([[maybe_unused]] Event event)
{
    WorldPacket p(event.getPacket());
    p.rpos(0);
