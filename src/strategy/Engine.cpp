...
bool ActionExecutionListeners::Before(Action* action, [[maybe_unused]] Event event)
{
    bool result = true;
...
void ActionExecutionListeners::After(Action* action, bool executed, [[maybe_unused]] Event event)
{
    for (std::list<ActionExecutionListener*>::iterator i = listeners.begin(); i != listeners.end(); i++)
    {
...
bool ActionExecutionListeners::OverrideResult(Action* action, bool executed, [[maybe_unused]] Event event)
{
...
bool ActionExecutionListeners::AllowExecution(Action* action, [[maybe_unused]] Event event)
{
    bool result = true;
    for (std::list<ActionExecutionListener*>::iterator i = listeners.begin(); i != listeners.end(); i++)
    {
...
bool Engine::DoNextAction([[maybe_unused]] Unit* unit, [[maybe_unused]] uint32 depth, bool minimal)
{
    LogAction("--- AI Tick ---");
    ...
    while (++iterations <= iterationsPerTick)
    {
        ...
        [[maybe_unused]] Event event = basket->getEvent();
...
bool Engine::MultiplyAndPush(NextAction** actions, float forceRelevance, bool skipPrerequisites, [[maybe_unused]] Event event,
                             char const* pushType)
{
    bool pushed = false;
    if (actions)
...
ActionResult Engine::ExecuteAction(std::string const name, [[maybe_unused]] Event event, std::string const qualifier)
{
    bool result = false;
...
void Engine::ProcessTriggers(bool minimal)
{
    ...
    for (std::vector<TriggerNode*>::iterator i = triggers.begin(); i != triggers.end(); i++)
    {
        ...
        if (testMode || trigger->needCheck(now))
        {
            ...
            [[maybe_unused]] Event event = trigger->Check();
            if (pmo)
                pmo->finish();
    ...
    for (std::vector<TriggerNode*>::iterator i = triggers.begin(); i != triggers.end(); i++)
    {
        ...
        [[maybe_unused]] Event event = fires[trigger];
...
void Engine::PushAgain(ActionNode* actionNode, float relevance, [[maybe_unused]] Event event)
{
...
bool Engine::ListenAndExecute(Action* action, [[maybe_unused]] Event event)
{
    bool actionExecuted = false;
